...
public final class PingPassthroughMigration implements ConfigurationMigration {
  ...
  public boolean shouldMigrate(final CommentedFileConfig config) {
    ...
    // so we need to check if a migration is needed without using `configVersion(config) < 2.9`.
    Object pingPassthrough = config.get("ping-passthrough");
    return pingPassthrough == null || pingPassthrough instanceof String;
  }
