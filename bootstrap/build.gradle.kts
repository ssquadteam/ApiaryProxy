...
val generateLibraries by tasks.registering {
    ...
    val relocatedLibrariesManifest = proxyProject.tasks.named("relocatedLibrariesManifest")
    ...
    dependsOn(relocatedLibrariesManifest)

    val proxyRuntimeArtifacts = proxyRuntimeClasspath.incoming.artifacts

    inputs.files(proxyRuntimeArtifacts.artifactFiles)
    inputs.files(relocatedLibrariesManifest)
    ...
    val relocatedManifestFile = relocatedLibrariesManifest.map { it.outputs.files.singleFile }
    val runtimeArtifacts = proxyRuntimeArtifacts.resolvedArtifacts
    val repositories = runtimeRepositories
    ...
    doLast {
        ...
        repositories.forEach { lines += "repo\t$it" }
        ...
        val relocatedKeys = relocatedManifestFile.get()
            .readLines()
            .filter { it.isNotBlank() }
            .toSet()

        for (artifact in runtimeArtifacts.get()) {
            ...
            if ("${id.group}:${id.module}" in relocatedKeys) {
                continue
            }
            val fileName = artifact.file.name
            val extension = fileName.substringAfterLast('.', "jar")
            val baseName = fileName.removeSuffix(".$extension")
            val classifier = baseName.removePrefix("${id.module}-${id.version}")
                .removePrefix("-")
                .ifEmpty { "-" }
            ...
            if (id.version.endsWith("-SNAPSHOT")) {
                ...
                lines += listOf(
                    "dep", "EMBEDDED", id.group, id.module, id.version, extension, classifier,
                    sha256(target), "jar-in-jar/$fileName"
                ).joinToString("\t")
            } else {
                lines += listOf(
                    "dep", "MAVEN", id.group, id.module, id.version, extension, classifier,
